...
void load_sm_elf(void* blob, size_t size, kernel_elf_info* info)
{
  /*
  Blob layout (ELF):
  - ELF Header
  ...
  */
  ...
  if (sizeof(*eh) > size)
    goto fail;
    // If the reported size of header is larger than requested allocated size, fail

  if ( !(eh->e_ident[0] == '\177' && eh->e_ident[1] == 'E' && eh->e_ident[2] == 'L'    && eh->e_ident[3] == 'F') )
    goto fail;
    // If the thing doesn't begin with an ELF identifier, fail

  if (IS_ELF64(*eh) != (sizeof(uintptr_t) == 8))
    goto fail;
    // if the elf and the arch don't agree on 64-bittedness, fail

  size_t phdr_size = eh->e_phnum * sizeof(Elf_Ehdr);
  Elf_Phdr* ph = blob + eh->e_phoff; // Page handler???
...
fail:
    die("failed to load sanctum payload");
}
